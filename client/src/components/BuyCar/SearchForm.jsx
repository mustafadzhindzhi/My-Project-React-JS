...
const SearchForm = ({ searchCriteria, onSearch }) => {
  ...
  const handleChange = (event) => {
    ...
    const newFormData = { ...formData, [name]: newValue, category: newCategory };
...
const handleSliderChange = (event) => {
  ...
  {['New', 'Used', 'For parts'].map((category) => (
    <label
      key={category}
      className={`check ${formData.category === category ? 'selected' : ''}`}
  ...
  <h2 style={{ fontSize: "1.3vw", fontWeight: 'bold', textAlign:'left' }}>Price Range</h2>
  ...
  />
          ...
          <fieldset className="comfort-fieldset">
            <div className="comfort-toggle" onClick={toggleComforts}>
              ...
              Comforts <span className={`arrow ${comfortsVisible ? 'arrow-up' : 'arrow-down'}`}></span>
              </button>
            </div>
            <div className={`comfort-list ${comfortsVisible ? 'visible' : 'hidden'}`}>
                {['Leather seats', 'Climate control', 'Auto/Start Stop Function', 'Bluetooth', 'Steptronic, Tiptronic', 'Bordcomputer'].map((comfort, index) => (
                  <label key={index}>
                    <input
                      type="checkbox"
                      name="comforts"
